...
**Pegadinha do cache:** plugins de marketplace são **copiados** para `~/.claude/plugins/cache`, não usados in-place. Isso significa que `dist/` (compilado) e `node_modules` precisam viajar JUNTO no pacote — não dá pra contar com `npm install`/`build` na máquina do usuário. Para teste local via `--plugin-dir` ou marketplace local, isso não morde (usa os arquivos no lugar). **Resolvido em 2026-07-19 versionando o bundle — ver a seção "Bundle do plugin é COMMITADO" abaixo.**
...
## Bundle do plugin é COMMITADO — e o `.gitignore` que quebrava isso (2026-07-19)

**Decisão:** `web-eyes/plugin/dist/` e `web-eyes/plugin/node_modules/` (~61MB) são **versionados no git**. Contra-intuitivo (commitar `node_modules` é tabu), mas obrigatório aqui: o Claude Code **copia** o plugin pro cache e roda `node dist/index.js` lá, **sem build e sem install** — o que não estiver commitado simplesmente não existe pro usuário. A alternativa (pedir `npm install` no clone) contraria o requisito de instalação de uma linha do BRIEFING. Custo aceito: ~61MB no histórico, e a cada release rodar `prepare-plugin.mjs` **e commitar o resultado**.

**A armadilha que custou tempo — `.gitignore` aninhado vence o da raiz.** O `.gitignore` da RAIZ já tinha as regras de negate (`!web-eyes/plugin/dist/**` etc.) e parecia certo. Mas o bundle continuava invisível ao git, e `git ls-files` retornava **zero** arquivos. Causa: existe um **segundo `.gitignore` em `web-eyes/`** com `dist/` e `node_modules/` sem negate nenhum — e o `.gitignore` mais profundo tem **precedência** sobre o da raiz. Os negates da raiz nunca chegavam a ser avaliados. **Diagnóstico:** `git check-ignore -v <arquivo>` diz o arquivo E a linha exata que ignoram — foi o que apontou o `web-eyes/.gitignore:2` em vez do da raiz. Use isso antes de mexer em regra de ignore; ler o `.gitignore` da raiz e concluir não basta.

**Segunda sutileza (essa é do git, não do projeto):** não adianta `!plugin/dist/**` sozinho — o git **não desce em diretório excluído**, então reincluir um arquivo exige reincluir cada diretório-pai ANTES. Por isso as regras vêm em pares (`!plugin/dist/` e depois `!plugin/dist/**`). Remover a linha do diretório "porque o `**` já cobre" quebra tudo de novo, silenciosamente.
