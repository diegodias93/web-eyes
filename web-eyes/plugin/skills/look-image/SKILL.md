...
Steps:
1. Call the MCP tool `capture_screenshot` (from the `web-eyes` server).
...
Use this skill when visuals matter. To just read text (cheaper) use `/look-text`; for HTML structure use `/look-dom`.
