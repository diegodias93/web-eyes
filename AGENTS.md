...
- **Skill** = *como eu faço algo* — um procedimento que vira `/<nome>`. Vive em `.Codex/skills/<nome>/SKILL.md` (uma pasta — pode levar scripts junto). Nome em minúsculas, números e hífens (padrão Agent Skills).
...
- Skills antigas em `.Codex/commands/<nome>.md` ainda funcionam, mas o formato preferido é `.Codex/skills/`. Não crie novas em `commands/`.

**Tudo é por projeto e versionado** — skills moram em `.Codex/` deste repo, não no global. Capacidade específica do sistema fica com o sistema.
...
Simplicidade, modularização, mudanças cirúrgicas e execução orientada a objetivos vivem na skill **`como-desenvolver`** (`.Codex/skills/`). **Ao escrever ou editar código, invoque a skill `como-desenvolver` e siga-a** — não dependa de eu lembrar de cor. Você também pode chamar `/como-desenvolver` a qualquer momento.
