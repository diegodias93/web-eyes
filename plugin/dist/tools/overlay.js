 ...
 */
function buildOverlay(id, binding) {
    ...
    const BTN = "cursor:pointer;border-radius:8px;padding:8px 16px;min-width:78px;" +
 ...
 */
function setOverlayBusy(id, action) {
 ...
 */
export function overlayInitScript() {
...
export async function removeOverlay(page) {
    await page
        ...
        window["__webEyesKeyCleanup"]?.();
        window["__webEyesKeyCleanup"] = undefined;
    }, OVERLAY_ID)
        .catch(() => { });
}
/**
 ...
 * no external deps.
 */
export async function setOverlayBusyOn(page, action) {
 ...
 */
export async function setOverlayHiddenOn(page, hidden) {
    await page
        .evaluate(([id, h]) => {
        const host = document.getElementById(id);
        if (host)
            host.style.visibility = h ? "hidden" : "visible";
    }, [OVERLAY_ID, hidden])
        .catch(() => { });
}
